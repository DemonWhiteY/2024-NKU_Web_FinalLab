...
/**
 * @file
 * 本文件是一个Yii框架的视图文件，用于构建网页的主内容部分。
 * 
 * 该视图文件的主要功能是展示一个团队的主页，内容涵盖团队介绍、服务展示、博客文章以及留言板等多个部分。具体来说：
 * 
 * 1. **页面标题**: 设置页面的标题为"My Yii Application"。
 * 
 * 2. **幻灯片区域**: 使用Swiper库实现的幻灯片区域，通过背景图与文本内容呈现欢迎信息，吸引访问者的注意力。
 * 
 * 3. **跟踪查询表单**: 提供一个跟踪查询的表单，允许用户选择货物类型和产品类型，并输入追踪号码进行查询，增强用户体验。
 * 
 * 4. **关于我们部分**: 简要介绍团队成员及其背景，包括团队名称和各成员的基本信息，突显团队的专业性和组织架构。
 * 
 * 5. **个人风采展示**: 利用Swiper展示团队成员的个人风采，使得内容呈现更加生动。
 * 
 * 6. **最新博客文章**: 列出团队的最新博客文章，提供文章标题、时间和简要内容，鼓励用户点击阅读更多信息。
 * 
 * 7. **留言板**: 提供用户留言的功能区域，展示用户反馈和评价，增强与用户的互动。
 * 
 * 该视图文件的设计旨在以清晰和美观的方式展示信息，同时确保用户能够轻松找到他们想要的内容。使用了现代化的网页设计元素，包括响应式布局、动态内容滑块和交互式表单，提升用户体验。
 * 
 * 该页面是团队在互联网数据库开发课程中的一部分，展示了团队的努力成果和专业技能，适合用于学术展示、项目推广或个人作品集。
 */
